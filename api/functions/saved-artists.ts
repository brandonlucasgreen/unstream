...
async function handleSave(user: { userId: string; email: string }, body: Record<string, unknown>, client: ReturnType<typeof getServiceClient>) {
  ...
  // This prevents stored XSS via malicious slugs reaching the edge function SSR — the character
  // class is what does that work, by admitting nothing that can break out of HTML.
  //
  ...
  // from it, and the /u/:handle page would then link a /a/{uuid} URL that 404s. The React artist
  // page did exactly this, and its 400 is the only reason the bug was visible rather than silent.
  const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
  if (UUID_REGEX.test(artistSlug)) {
    return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'artistId must be an artist slug, not an id' }) };
  }
