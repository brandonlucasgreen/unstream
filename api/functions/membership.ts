// Open House membership state: the rules for "is this user a member?", the mapping from Stripe
// objects to a `memberships` row, and the table's reads and writes.
//
// Spec: docs/specs/open-house-membership-spec.md §8. The table is server-only (RLS on, no
 ...
 */
export function summariseMemberships(rows: MembershipRow[], now: Date = new Date()): OpenHouseLive {
