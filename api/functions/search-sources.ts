...
export async function handler(event: { queryStringParameters?: Record<string, string>; headers?: Record<string, string> }) {
  ...
  try {
    ...
    // exact name, so a claimed artist merely containing it is someone else.
    const claimedByNamePromise: Promise<AggregatedResult[]> = mode === 'exact'
      ? Promise.resolve([])
      : findClaimedArtistSlugsByName(normalizedQuery)
