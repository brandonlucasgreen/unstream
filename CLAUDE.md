...
Edge functions in `api/edge/` handle SSR for SEO, routed in `netlify.toml`: `og-metadata` (`/`), `artist-page` (`/artist/*`), `claimed-artist-page` (`/a/*`), `artist-directory-page` (`/artists`), `guide-page` (`/guides/*`), `noscript-search` (`/search`).
