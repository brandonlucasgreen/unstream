...
**Auto-deploy:** `supabase-migrate.yml` runs `supabase db push --linked` on every push to `main` touching `supabase/migrations/`. Secrets: `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD`. A `deadlock detected` failure there is transient — each migration is one transaction, so it rolled back whole and was not recorded; the push step retries three times, and re-running the job by hand is safe. It bites migrations that lock several tables (policy rewrites) merged during the sweep's 01:00 / 13:00 UTC windows. On 2026-09-13 the re-run then said "up to date" while the schema showed the change applied, so a second applicator had raced the workflow — verify against `pg_policies` / `pg_indexes`, not the workflow's colour, and make sure only one thing applies migrations.
