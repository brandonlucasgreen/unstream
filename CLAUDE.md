...
**Auto-deploy:** `supabase-migrate.yml` runs `supabase db push --linked` on every push to `main` touching `supabase/migrations/`. Secrets: `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD`. A `deadlock detected` failure there is transient — each migration is one transaction, so it rolled back whole and was not recorded; the push step retries three times, and re-running the job by hand is safe. It bites migrations that lock several tables (policy rewrites) merged during the sweep's 01:00 / 13:00 UTC windows. On 2026-09-13 the other side of the deadlock was Supabase's GitHub integration applying the same file from `main` at the same moment; that setting is now off, and this workflow is the only applicator — keep it that way. Verify a migration against `pg_policies` / `pg_indexes`, not the workflow's colour.
