...
const elements = {
  ...
  savedProPrompt: document.getElementById('saved-pro-prompt'),
  // Settings tab
  licenseStatus: document.getElementById('license-status'),
  licenseInputSection: document.getElementById('license-input-section'),
...
async function init() {
  ...
  // Load saved artists and releases if pro
  if (isProUser) {
  ...
  try {
    ...
    if (data.valid) {
      await chrome.storage.sync.set({
        license: { key, status: 'active', validatedAt: Date.now() },
      });
      return true;
    }
  } catch (error) {
...
function updateLicenseUI() {
  ...
  if (isProUser) {
    ...
    statusText.textContent = 'Yearly Pass Active';
    elements.licenseStatus.classList.add('active');
    // Hide license input section when Yearly Pass is active
    elements.licenseInputSection.style.display = 'none';
    // Show release alerts section
    elements.releaseAlertsSection.classList.remove('hidden');
    // Show saved section, hide pro prompt
    elements.savedSection.classList.remove('hidden');
    elements.savedProPrompt.classList.add('hidden');
  } else {
    statusIcon.textContent = '○';
    statusText.textContent = 'Free Plan';
    elements.licenseStatus.classList.remove('active');
    // Show license input section
    elements.licenseInputSection.style.display = 'block';
    // Hide release alerts section
    elements.releaseAlertsSection.classList.add('hidden');
    // Hide saved section, show pro prompt
    elements.savedSection.classList.add('hidden');
    elements.savedProPrompt.classList.remove('hidden');
  }
}
