...
export function ResultCard({ result, defaultExpanded = true }: ResultCardProps) {
  ...
  const artistSlug = result.claimedSlug || result.id;
  const saved = isSaved(artistSlug);
    ...
    <div className="result-card group">
      ...
      >
        ...
        <div className="flex-shrink-0 flex items-center gap-1">
          {/* Save button */}
          <button
            onClick={(e) => { e.stopPropagation(); toggleSave(artistSlug, result.name, result.imageUrl); }}
