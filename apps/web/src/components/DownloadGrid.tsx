...
export function DownloadGrid() {
  ...
  if (isIOS) {
    ...
    // Android: "Add to Home Screen" (if PWA installable)
    if (isAndroid && isPWAInstallable) {
      buttons.push({
        key: 'android-install',
    ...
    // Chrome extension (any platform except iOS)
    if (isChrome) {
      buttons.push({
        key: 'chrome',
    ...
    // Firefox extension (any platform except iOS)
    if (isFirefox) {
      buttons.push({
        key: 'firefox',
    ...
    // macOS download (desktop Mac, not iOS)
    if (isMac && !isMobile) {
      buttons.push({
        key: 'macos',
  ...
  // Fallback: if no platform-specific buttons matched, show the relevant defaults
  if (buttons.length === 0) {
