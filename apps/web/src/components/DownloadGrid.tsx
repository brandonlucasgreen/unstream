...
export function DownloadGrid() {
  ...
  // Desktop: always show all 4 buttons. Mobile: show only relevant buttons.
  // iOS: "Add to Home Screen" + "Get iOS Shortcut"
  // Android: "Add to Home Screen" (if PWA installable) + browser extension
  // Desktop: all 4 (macOS, iOS Shortcut, Chrome, Firefox)
  const showAll = !isMobile;

  // Build the list of buttons to show based on platform/browser
  const buttons: {
    key: string;
    href?: string;
    onClick?: () => void;
  ...
  if (showAll) {
    // Desktop: always show all 4 buttons
    buttons.push(
      {
        key: 'macos',
        href: 'https://github.com/brandonlucasgreen/unstream/releases/latest',
        bg: 'bg-[#9CA3AF]',
        hoverBg: 'hover:bg-[#8B92A0]',
        shadow: 'shadow-[#9CA3AF]/20',
        icon: (
      ...
      },
      {
        ...
        bg: 'bg-[#34A853]',
        hoverBg: 'hover:bg-[#2d9249]',
        shadow: 'shadow-[#34A853]/20',
        icon: (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      ...
      },
      {
        ...
        bg: 'bg-[#FF6611]',
        hoverBg: 'hover:bg-[#e55b0e]',
        shadow: 'shadow-[#FF6611]/20',
        icon: (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    // iOS mobile: "Add to Home Screen" + "Get iOS Shortcut"
    buttons.push({
      key: 'ios-home-screen',
    ...
    // Android mobile: "Add to Home Screen" + relevant browser extension
    if (isPWAInstallable) {
      buttons.push({
        key: 'android-install',
    ...
    // If no mobile browser matched (rare), show both Chrome and Firefox
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-md mx-auto items-stretch">
        {buttons.map((btn) => {
          const className = `flex items-center justify-center gap-2 px-4 py-3 rounded-xl ${btn.bg} text-white ${btn.hoverBg} transition-colors font-medium shadow-lg ${btn.shadow} whitespace-nowrap`;
          ...
          return (
            <a
              ...
              className={className}
              onClick={() => {
                if (btn.key === 'macos') analytics.trackDownload();
                else if (btn.key === 'chrome') analytics.trackDownloadChrome();
                else if (btn.key === 'firefox') analytics.trackDownloadFirefox();
                else if (btn.key === 'ios-shortcut') analytics.trackDownloadIosShortcut();
              }}
            >
              {btn.icon}
              {btn.label}
