...
struct BrandIconShape: Shape {
    let path: String
    var viewBoxWidth: CGFloat = 24
    var viewBoxHeight: CGFloat = 24

    func path(in rect: CGRect) -> Path {
        let scaleX = rect.width / viewBoxWidth
        let scaleY = rect.height / viewBoxHeight
        let scale = min(scaleX, scaleY)
        let xOffset = (rect.width - viewBoxWidth * scale) / 2
        let yOffset = (rect.height - viewBoxHeight * scale) / 2
...
struct BrandIconPaths {
    ...
    // PeerTube uses a multi-color logo (3 triangles) — rendered as a special case in BrandIcon.body
    static let peertubeTop = "M0,0 L0,12 L9.12,6 Z"       // viewBox 0 0 18.24 24
    static let peertubeBottom = "M0,12 L0,24 L9.12,18 Z"
    static let peertubeRight = "M9.12,6 L9.12,18 L18.24,12 Z"
}
...
struct BrandIcon: View {
    ...
    var body: some View {
        if platform == "peertube" {
            PeerTubeIcon(size: size)
        } else if let path = pathForPlatform(platform) {
...
// MARK: - PeerTube Multi-Color Icon

struct PeerTubeIcon: View {
    let size: CGFloat

    var body: some View {
        ZStack {
            BrandIconShape(path: BrandIconPaths.peertubeTop, viewBoxWidth: 18.24, viewBoxHeight: 24)
                .fill(Color.primary)
            BrandIconShape(path: BrandIconPaths.peertubeBottom, viewBoxWidth: 18.24, viewBoxHeight: 24)
                .fill(Color.gray)
            BrandIconShape(path: BrandIconPaths.peertubeRight, viewBoxWidth: 18.24, viewBoxHeight: 24)
                .fill(Color(red: 241/255, green: 104/255, blue: 13/255))
        }
        .frame(width: size, height: size)
    }
}
